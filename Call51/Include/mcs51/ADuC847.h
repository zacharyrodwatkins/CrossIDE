/*-------------------------------------------------------------------------
   ADuC847.h: Register Declarations for Analog Devices
   ADuC845/ADuC847/ADuC848 (Based on datasheet Rev B, 02/2005 )

   Copyright (C) 2007, Jesus Calvino-Fraga / jesusc at ece.ubc.ca
   Edited        2011, Jonathan Armstrong  / jonathan.armstrong at mavs.uta.edu (9/2/2011)
...
SFR(CFG845, 0xAF); // ADuC845 Configuration SFR
SFR(CFG847, 0xAF); // ADuC847 Configuration SFR
SFR(CFG848, 0xAF); // ADuC848 Configuration SFR
...
SFR(EWAIT, 0x9F); // Dictates the number of wait states for the MOVX instruction (see pg. 23 in datasheet).
...
SFR(I2CADD1, 0xF2); // I2C Address Register 2.
...
SFR(IEIP2, 0xA9); // Secondary Interrupt Enable Register
    ...
    #define PSI   0x10 // Priority for SPI/I2C interrupt.
...
SFR(PWMCON, 0xAE); //PWM control.
    #define PWM2    0x40    // PWM Mode Selection bit 2.
    #define PWM1    0x20    // PWM Mode Selection bit 1.
    #define PWM0    0x10    // PWM Mode Selection bit 0.
    #define PWS1    0x08    // PWM Clock Source Divider bit 1.
    #define PWS0    0x04    // PWM Clock Source Divider bit 0.
    #define PWC1    0x08    // PWM Clock Source Selection bit 1.
    #define PWC0    0x08    // PWM Clock Source Selection bit 0.
...
SFR(PSW, 0xD0); // Program Status Word.
   ...
   SBIT(FI,  0xD0, 1); // User-Defined Flag.
...
SFR(ADCMODE, 0xD1); // ADC Mode Register - Used to control the operational mode of both ADCs.
    #define REJ60   0x40    // Automatic 60 Hz notch select bit.
    #define ADC0EN  0x20    // Primary ADC enable.
    #define ADC1EN  0x10    // Auxiliary (ADuC845 only) ADC Enable.
    #define CHOP    0x08    // Chop Mode Disable.
    #define MD2     0x04  // Primary and Auxiliary (ADuC845 only) ADC Mode Bits.
    #define MD1     0x02
    #define MD0     0x01
    
SFR(ADC0CON1, 0xD2); // Primary ADC control register
    #define BUF1    0x80    // Buffer Configuration Bits.
    #define BUF0    0x40    // Buffer Configuration Bits.
    #define UNI     0x20    // Primary ADC Unipolar Bits.
    #define RN2     0x04    // Primary ADC Range Bits.
    #define RN1     0x02    // Primary ADC Range Bits.
    #define RN0     0x01    // Primary ADC Range Bits.
    
SFR(ADC1CON, 0xD3); // (AUXILIARY ADC CONTROL REGISTER) (ADuC845 ONLY)
    #define AXREF   0x40    // Auxiliary ADC External Reference Bit.
    #define AUNI    0x20    // Auxiliary ADC Unipolar Bit.
    #define ACH3    0x08    // Auxiliary ADC Channel Select Bits.
    #define ACH2    0x04    // Auxiliary ADC Channel Select Bits.
    #define ACH1    0x02    // Auxiliary ADC Channel Select Bits.
    #define ACH0    0x01    // Auxiliary ADC Channel Select Bits.
...
SFR(SF, 0xD4);      // ADC Sinc Filter Control Register
SFR(ICON, 0xD5);    // Excitation Current Sources Control Register.

SFR(ADCSTAT,  0xD8);
    SBIT(RDY0,   0xD8, 7);  // Ready Bit for the Primary ADC.
    SBIT(RDY1,   0xD8, 6);  // Ready Bit for Auxiliary (ADuC845 Only) ADC.
    SBIT(CAL,    0xD8, 5);  // Calibration Status Bit.
    SBIT(NOXREF, 0xD8, 4);  // No External Reference Bit (only active if primary or auxiliary (ADuC845 only) ADC is active.)
    SBIT(ERR0,   0xD8, 3);  // Primary ADC Error Bit.
    SBIT(ERR1,   0xD8, 2);  // Auxiliary ADC Error Bit. Valid on the ADuC845 only.
    
SFR(ADC0L, 0xD9);   // Primary ADC Conversion Result - Low Byte (Not Available on ADuC848).
SFR(ADC0M, 0xDA);   // Primary ADC Conversion Result - Middle Byte.
SFR(ADC0H, 0xDB);   // Primary ADC Conversion Result - High Byte.

SFR(ADC1L, 0xDE);   // Primary ADC Conversion Result - Low Byte (ADuC845 Only).
SFR(ADC1M, 0xDC);   // Primary ADC Conversion Result - Middle Byte (ADuC845 Only).
SFR(ADC1H, 0xDD);   // Primary ADC Conversion Result - High Byte (ADuC845 Only).

SFR(OF0L, 0xE1);    // Primary ADC 24bit offset calibration coefficient (Not Available on ADuC848).
SFR(OF0M, 0xE2);    // Primary ADC 24bit offset calibration coefficient.
SFR(OF0H, 0xE3);    // Primary ADC 24bit offset calibration coefficient.

SFR(OF1L, 0xE4);    // Primary ADC 24bit offset calibration coefficient (ADuC845 Only).
SFR(OF1H, 0xE5);    // Primary ADC 24bit offset calibration coefficient (ADuC845 Only).

SFR(ADC0CON2, 0xE6);  //Primary ADC Channel Select Register
    #define XREF1 0x80    // Primary ADC External Reference Select Bit.
    #define XREF0 0x40    // Primary ADC External Reference Select Bit.
    #define CH3   0x08    // Primary ADC Channel Select Bits.
    #define CH2   0x04    // Primary ADC Channel Select Bits.
    #define CH1   0x02    // Primary ADC Channel Select Bits.
    #define CH0   0x01    // Primary ADC Channel Select Bits.
    
SFR(GN0L, 0xE9);    // Primary ADC 24-bit gain calibration coefficient is held in these three 8-bit registers (GN0L is not available on the ADuC848).
SFR(GN0M, 0xEA);
SFR(GN0H, 0xEB);
SFR(GN1L, 0xEC);    // Auxiliary ADC 16-bit gain calibration coefficient is held in these three 8-bit registers (ADuC845 Only).
SFR(GN1H, 0xED);
...
SFR(DACL, 0xFB);    // DAC Data Register Low Byte.
SFR(DACH, 0xFC);    // DAC Data Register High Byte.
SFR(DACCON, 0xFD); // DAC Control Register.
    #define DACPIN  0x10    // DAC Output Pin Select.
    #define DAC8    0x08    // DAC 8-Bit Mode Bit.
    #define DACRN   0x04    // DAC Ouput Range Bit.
